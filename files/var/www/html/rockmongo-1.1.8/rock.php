...
{
	$_SERVER["PHP_SELF"] = $_SERVER['HTTP_X_FORWARDED_PREFIX'] . $_SERVER["PHP_SELF"];
	$_SERVER["SCRIPT_NAME"] = $_SERVER['HTTP_X_FORWARDED_PREFIX'] . $_SERVER["SCRIPT_NAME"];
	$_SERVER["DOCUMENT_URI"] = $_SERVER['HTTP_X_FORWARDED_PREFIX'] . $_SERVER["DOCUMENT_URI"];
...
function rock_name_to_java($name) {
	...
	$name = preg_replace_callback("/_([a-zA-Z])/", $f, $name);
	return $name;
}
...
function rock_array_get(array $array, $keys) {
	...
	if (!is_array($keys)) {
		if (strstr($keys, "`")) {
			$fn = function ($match){ return str_replace(".", "\.", $match[1]); };
			$keys = preg_replace_callback("/`(.+)`/U", $fn, $keys);
		}
